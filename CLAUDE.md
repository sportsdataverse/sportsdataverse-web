...
- **Community:** `/join`, `/survey`, `/join/confirmed`; `POST /api/join`, `POST /api/survey`, `GET /api/join/confirm`; review queue at `/platform/people` — **any org member**, because vouching is what the invite-only server is for; it deliberately does NOT live under `/platform/admin`, whose layout gates Keys/Errors/Traffic on the `admin` role — with `GET /api/platform/people` (`?view=queue|unsynced|all` — for a non-admin, `all` is narrowed server-side (`peopleViewFilter`, `lib/peopleRow.ts`) to people who asked for Discord; only an org admin gets every name and email), `GET /api/platform/people/population` (counts-only aggregate for the Population tab), and `POST /api/platform/people/[id]/[action]` (`action` = `approve|decline|requeue|resend|retry-sync|delete`; every action is member-level except `delete`, which is irreversible and stays admin-only). Sticker requests are admin-only from the start, not just on delete: `/platform/admin/stickers`, `GET /api/platform/admin/stickers`, and `POST /api/platform/admin/stickers/[id]/[action]` (`ship|cancel`) all sit behind the same `admin` role gate as Keys/Errors/Traffic — see `frontend/lib/stickers.ts`. Questions are data in `frontend/content/survey.ts`; engine + handlers in `frontend/lib/{survey,join,review,discord}.ts`. `/survey` is identified (name + email) since 2026-09-25; every questionnaire submission appends a `responses` document (`lib/responses.ts`) and `people` holds the latest identity (`lib/identity.ts`). The admin Community browser is a further-gated tool, separate from the review queue: `/platform/admin/community` (filter/search/aggregate/cross-tab over every person) and `/platform/admin/community/[id]` (one person's identity and full `responses` history, plus the do-not-contact toggle), backed by `GET /api/platform/admin/community`, `GET /api/platform/admin/community/[id]`, `POST /api/platform/admin/community/[id]/do-not-contact`, and `GET /api/platform/admin/community/export` (CSV) — every one of those four routes requires the `admin` role (`lib/community.ts`, `lib/communityData.ts`). See `frontend/SETUP-community.md`.
- **Package visibility:** in the `packages` collection, `published: false` is not a hide flag — every legacy package carries it and is public. A package is publicly visible when it has no `submittedBy`, or when `published === true` (`lib/packageVisibility.ts`'s `PUBLIC_PACKAGE_FILTER`/`isPubliclyVisible`). Any new public reader of `packages` must filter with it — a bare `find({})` publishes a stranger's unapproved submission — and `test/packageVisibilityReaders.test.ts` checks this for every reader except the member CMS at `/packages/manage`. It is part of `npm run test:lib`, which the `lib` job in `.github/workflows/unit-tests.yml` runs on every pull request — its own workflow with no paths filter, so it can be a required check.
- **Sticker address visibility:** a postal address is read only by `listOpenStickerRequests` (`frontend/lib/stickers.ts`), whose only caller is the admin sticker route — never add another reader, and never put an address on `people`.
- **Identity visibility:** a person's location, answers and `responses` are never shown to non-admin members. The member review queue (`lib/peopleRow.ts`) shows affiliations and socials only for people awaiting a Discord decision, labeled self-reported — keep it that way. Full per-person browsing (every name and email, in every status) is admin-only: a non-admin's People-tab "All" view (`peopleViewFilter`) is narrowed to Discord requesters (`wants.discord: true`) so a survey-only or newsletter-only respondent is never listed there; Queue and Unsynced are unaffected, since the actions on those views already require the same `wants.discord`/newsletter gate. A person's full record (location, answers, `responses` history) is read for display only through `lib/communityData.ts`, and only from the admin Community routes under `app/api/platform/admin/community/**` — `test/personDataReaders.test.ts` is a source scan that fails if that module is imported anywhere else, if the `responses` collection name is named outside it and `lib/responses.ts`, or if a community admin route is missing `requireAdminApp`. Its CSV export (`lib/communityData.ts`'s `exportable`/`toCsv`) skips anyone marked do-not-contact, anyone with no email on file, anyone who hasn't made an identified `/join` or `/survey` submission since the contact notice was added (a footer-only newsletter signup, or a `/join` from before that date, gave no such consent), anyone who has unsubscribed from the newsletter, and reserved test addresses, and every export — and every do-not-contact change, on or off — is recorded in the `admin_audit` collection.
- **Population tab privacy:** never add a cross-tab (e.g. role × sport) or a free-text breakdown to `/platform/people`'s Population tab. It shows single-variable counts of closed-list, low-sensitivity answers, and those are not anonymous: the queue shows no answers, but a member comparing totals before and after one new joiner can read off that person's answers — a risk accepted for these counts. A cross-tab or a free-text breakdown would make that far worse.
...
- **Mongo `people` is written before any outbound Resend/Discord call, never after** (`lib/join.ts`,
  ...
  **`/join`'s Resend calls (the single opt-in contact create/refresh, the double opt-in
  confirmation email, the Discord invite email, and the sticker got-it email) now run after the
  response via Next's `after()`** (`JoinDeps.defer`, wired in `app/api/join/route.ts`), so the
  route doesn't even await them — the person is already saved and the reply already sent before
  any of them fire. That includes `syncContact`'s single opt-in path: `newsletter.resendContactId`
  now lands strictly AFTER the reply, so a person can briefly show as unsynced in the People tab
  right after signing up — that is expected, not a bug, and resolves itself once the deferred
  task runs. A failure shows up only in the log, never in the reply or its timing; don't try to

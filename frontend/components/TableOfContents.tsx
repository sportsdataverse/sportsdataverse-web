      ...
      {tableOfContents.length > 0 && (
        <>
          <div
            ...
            } md:left-0 md:opacity-100 md:max-w-[35%] lg:max-w-[30%]  transition-all duration-500 flex flex-col gap-1 !pb-[100px] overflow-y-scroll p-10 md:p-14 h-screen fixed w-full font-barlow bg-background text-foreground z-50 `}
          >
            {/* TOC Search Bar */}
            <div className="relative w-full group mx-auto text-muted-foreground  rounded-md -ml-[5px]">
              <CgSearch className="ml-3 w-5 h-5 absolute top-[50%] -translate-y-1/2 z-10" />

              <input
                className="w-full px-5 py-2 pl-10 transition duration-200 bg-card rounded-md shadow outline-none ring-1 ring-border hover:ring-ring"
                type="text"
                value={searchValue}
          ...
          <button
            ...
            className="fixed bottom-0 z-50 w-full py-2 font-medium bg-primary text-primary-foreground outline-none md:hidden"
          >
            Table of Contents
          </button>
        </>

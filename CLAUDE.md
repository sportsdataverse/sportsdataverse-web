...
- **Population tab privacy:** never add a cross-tab (e.g. role × sport) or a free-text breakdown to `/platform/people`'s Population tab — single-variable counts are safe because a member can already see each person's record in the queue, but a cross-tab of small categories can identify someone.
